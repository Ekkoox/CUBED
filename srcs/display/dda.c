...
/*   Updated: 2025/08/29 21:45:54 by dsatge           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_hit_calc(t_cubed *cube)
{
	if (cube->ray->dda == VERTICAL)
	{
		...
		cube->ray->y_hit = cube->ray->player_pos_y + (cube->ray->dist_line_x * cube->ray->ray_rad_y);
	}
	if (cube->ray->dda == HORIZONTAL)
	{
		cube->ray->y_hit = cube->ray->move_spot_y;
		cube->ray->x_hit = cube->ray->player_pos_x + (cube->ray->dist_line_y * cube->ray->ray_rad_x);
	}	
}
...
void	dist_line_calc(t_cubed *cube)
{
	...
	{
		cube->ray->dist_line_x = (cube->ray->move_spot_x + 1.0 - cube->ray->player_pos_x) * cube->ray->delta_x;
		cube->ray->step_x = 1;
	}
	else
	{
		cube->ray->dist_line_x = (cube->ray->player_pos_x - cube->ray->move_spot_x) * cube->ray->delta_x;
		cube->ray->step_x = -1;
	}
	if (cube->ray->ray_rad_y > 0)
	{
		cube->ray->dist_line_y = (cube->ray->move_spot_y + 1.0 - cube->ray->player_pos_y) * cube->ray->delta_y;
		cube->ray->step_y = 1;
	}
	else
	{
		cube->ray->dist_line_y = (cube->ray->player_pos_y - cube->ray->move_spot_y) * cube->ray->delta_y;
		cube->ray->step_y = -1;
	}	
}
